   ...
   if(darkCounter < darkCounterLimit){
    ...
    rect(0,0,width,height);
     ...
     if(stage > -1){
         ...
         text("Spektrum", width/2-80, height/2);
     }
...
/* @pjs preload="data/BulletStandard.png"; 
 */
...
class Bullet{
 ...
 int timeOut = 24 * 2;
 float alpha;
 PImage img = loadImage("data/BulletStandard.png");
...
/* @pjs preload="data/player_ship.png"; 
 */
...
class Ship{
 ...
 float drag = .9;
 float r = 15;
 PImage img = loadImage("data/player_ship.png");
 ...
 void update(ArrayList<Exhaust> exhaust, ArrayList<Exhaust> fire){
   ...
   rotate2D(below, heading2D(rotation)+(3*PI)/2);
   below.add(position);
